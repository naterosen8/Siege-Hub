    ...
    <div style={{ maxWidth: 1080, margin: "0 auto", padding: "70px 24px" }}>
      ...
      <Panel>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
          <Link to={`/operators/${encodeURIComponent(a)}`} style={{ border: `1px solid ${C.line}`, borderRadius: 3, padding: 14, display: "block" }}>
            <Tag tone="atk">{a}</Tag>
            <div style={{ marginTop: 10, fontSize: 13, color: C.mute }}>{atk.role} · {atk.gadget}</div>
          </Link>
          <Link to={`/operators/${encodeURIComponent(d)}`} style={{ border: `1px solid ${C.line}`, borderRadius: 3, padding: 14, display: "block" }}>
            <Tag tone="def">{d}</Tag>
